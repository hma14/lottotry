...
##    outfile.close()
  ...
  def dbLastDrawNumber(self):
    dic = { "lottery":0, "lottomax":1, "BC49":2, "FloridaLotto":3, "MegaMillions":4, "PowerBall":5 }
  ...
  def searchDrawNumbers(self):
    if self.dbTable == 'lottery':
      ...
      match = re.search(r'(<span class=\'results\'>)([\d*,\s]+)(\sPowerball:*\s)(\d*)(,.+)(<.+)', file)
  ...
  def dbInsert(self):
    ...
    if self.dbTable == 'FloridaLotto' or self.dbTable == 'MegaMillions' or self.dbTable == 'PowerBall':
      ddate = self.searchDrawDate()
    else:
    ...
    dn += 1
    if self.dbTable == 'lottery' or self.dbTable == 'BC49' or self.dbTable == 'FloridaLotto':
