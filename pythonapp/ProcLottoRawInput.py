...
##
  ...
  def reverseOrder(self):
    infile = open("tmp2.txt", "r")
    outfile = open("tmp3.txt", "w")
    stack = Stack()
  ...
  def removeHyphen(self):
    rawDataFile = open("MegaMillions_RawData.txt", "r")
    outDataFile = open("tmp.txt", "w")
...
##  else:
##    tblName = 'BC49'
##  lottoDataInput = ProcLottoRawInput(tblName)

##  lottoDataInput.dbInsert()
##  lottoDataInput.dbSelectAll()
##  lottoDataInput.dbClose()
  lottoDataInput = ProcLottoRawInput("")
  lottoDataInput.removeHyphen()
  lottoDataInput.removeMultipleSpaces()
  lottoDataInput.reverseOrder()
